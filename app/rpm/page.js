...
function RpmInner() {
  ...
  return (
    <>
      ...
      {conn ? (
        <MotionFade>
          <div className="mb-6 rounded-2xl border border-secondary/15 px-4 py-4 sm:px-5 text-sm"
      ...
      {/* Hero: 3 gauges */}
      <div className="grid gap-4 sm:gap-5 sm:grid-cols-2 lg:grid-cols-3 mb-6">
        <MotionFade delay={0}>
          <Card className="rounded-2xl border-nobel/40 shadow-soft h-full" style={{ background: 'linear-gradient(135deg, #fff 55%, #FFF3EC)' }}>
            <CardContent className="p-4 sm:p-5 flex flex-col h-full">
              <h3 className="font-display text-base font-semibold text-secondary">Community Connection Index</h3>
              <p className="text-xs text-muted-foreground mb-1">Blended: openness · connections · chats · events</p>
        ...
        <MotionFade delay={0.05}>
          <Card className="rounded-2xl border-nobel/40 shadow-soft h-full">
            <CardContent className="p-4 sm:p-5 flex flex-col h-full">
              <h3 className="font-display text-base font-semibold text-secondary">Residents Open to Meeting</h3>
              <p className="text-xs text-muted-foreground mb-1">Share of residents welcoming connections</p>
              {conn ? <ConnectionGauge value={conn.openPct} max={100} label="percent" /> : null}
            </CardContent>
          </Card>
        </MotionFade>
        <MotionFade delay={0.1}>
          <Card className="rounded-2xl border-nobel/40 shadow-soft h-full sm:col-span-2 lg:col-span-1">
            <CardContent className="p-4 sm:p-5 flex flex-col h-full">
              <h3 className="font-display text-base font-semibold text-secondary">Team Average Rating</h3>
              <p className="text-xs text-muted-foreground mb-1">APM viewing performance across the region</p>
      ...
      {/* KPI strip */}
      <div className="grid gap-4 grid-cols-2 lg:grid-cols-4 mb-6 items-stretch">
        <MotionFade delay={0} className="h-full"><StatCard icon={HeartHandshake} label="Connections made" value={conn?.totalConnections ?? '—'} badge="▲ 12%" /></MotionFade>
        <MotionFade delay={0.05} className="h-full"><StatCard icon={MessageCircle} label="Active conversations" value={conn?.activeChats ?? '—'} badge="▲ 8%" /></MotionFade>
        <MotionFade delay={0.1} className="h-full"><StatCard icon={PartyPopper} label="Events sparked" value={conn?.eventsSparked ?? '—'} badge="▲ 3" /></MotionFade>
        <MotionFade delay={0.15} className="h-full"><StatCard icon={CalendarCheck} label="Viewings this month" value={stats?.viewingsThisMonth ?? '—'} hint={stats ? `${stats.totalViewings} in 90 days` : ''} /></MotionFade>
      </div>
      ...
      <div className="grid gap-5 lg:grid-cols-2 mb-6">
        <MotionFade delay={0.1}>
          <Card className="rounded-2xl border-nobel/40 shadow-soft h-full">
            <CardContent className="p-4 sm:p-5">
              <h3 className="font-display text-base font-semibold text-secondary">Resident Engagement Funnel</h3>
              <p className="text-xs text-muted-foreground mb-2">From signup to active community member</p>
        ...
        <MotionFade delay={0.15}>
          <Card className="rounded-2xl border-nobel/40 shadow-soft h-full">
            <CardContent className="p-4 sm:p-5">
              <h3 className="font-display text-base font-semibold text-secondary">Connections Over Time</h3>
              <p className="text-xs text-muted-foreground mb-2">Cumulative resident connections</p>
      ...
      <div className="grid gap-5 lg:grid-cols-2 mb-6">
        <MotionFade delay={0.1}>
          <Card className="rounded-2xl border-nobel/40 shadow-soft h-full">
            <CardContent className="p-4 sm:p-5">
              <h3 className="font-display text-base font-semibold text-secondary">Building Connection Leaderboard</h3>
              <p className="text-xs text-muted-foreground mb-4">Which communities are most engaged</p>
              <div className="space-y-3">
                {(data?.buildingLeaderboard || []).map((b, i) => (
                  <div key={b.name} className="flex items-center gap-3">
                    <span className="font-bold text-flamingo w-5">{i + 1}</span>
                    <span className="text-sm font-medium text-ink w-28 sm:w-40 truncate">{b.name}</span>
        ...
        <MotionFade delay={0.15}>
          <Card className="rounded-2xl border-nobel/40 shadow-soft h-full">
            <CardContent className="p-4 sm:p-5">
              <h3 className="font-display text-base font-semibold text-secondary">Top Resident Interests</h3>
              <p className="text-xs text-muted-foreground mb-2">What communities are into — event fuel</p>
      ...
      <MotionFade delay={0.1}>
        <Card className="rounded-2xl border-nobel/40 shadow-soft mb-6">
          <CardContent className="p-4 sm:p-5">
            <h3 className="font-display text-base font-semibold text-secondary">APM comparison</h3>
            <p className="text-xs text-muted-foreground mb-2">Overall average by manager</p>
            {apms.length ? <BarChart data={barData} max={5} height={260} /> : null}
          </CardContent>
        </Card>
      </MotionFade>

      {/* APM leaderboard table */}
      <MotionFade delay={0.2}>
        <Card className="rounded-2xl border-nobel/40 shadow-soft overflow-hidden">
          <CardContent className="p-0">
            <div className="px-4 sm:px-6 py-4 border-b border-nobel/40"><h3 className="font-display text-base font-semibold text-secondary">APM leaderboard</h3></div>
            {apms.length === 0 ? (
                  ...
                  <TableBody>
                    {apms.map(a => (
                      <TableRow key={a.id} className="cursor-pointer" onClick={() => router.push(`/rpm/apm/${a.id}`)}>
                        ...
                        <TableCell className="text-muted-foreground whitespace-nowrap">{a.propertyName}</TableCell>
